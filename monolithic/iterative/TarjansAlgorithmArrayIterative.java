...
public class TarjansAlgorithmArrayIterative {
    ...
    //@pure
    public boolean inSameSCC(int i, int j) {
        return reachMatrix[i][j] && reachMatrix[j][i];
    }
    ...
    /*@ normal_behaviour
      ...
      @             (\forall int j; i < j < GRAPH_SIZE; components[i] == components[j] ==> inSameSCC(i,j)) &&
      @             !(\exists int k; i < k < GRAPH_SIZE; components[i] != components[k] && inSameSCC(i,k))
      @         );
      @*/
